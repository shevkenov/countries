  ...
  const search = (items) => {
    return items.filter((item) => {
      if (item.region === region) {
        return searchedParmas.some(
          (param) =>
            item[param].toString().toLowerCase().indexOf(searchValue) > -1
        );
      } else if (region === "all") {
        return searchedParmas.some(
          (param) =>
        ...
        {isSuccess ? (
          <div className="card-container">
            {search(countries).map((country) => (
              <Link to={{pathname: `/${country.name}`, state:{country}}} key={country.name} style={{ textDecoration: 'none' }}>
